...
export default function CreateKPI({ onBack }) {
  ...
  const handleAddFormatting = () => {
    // abre diálogo ou adiciona regra padrão
    setFormatting(f => [...f, { id: Date.now(), label: 'Nova regra' }]);
  };

  const handleRemoveFormatting = (id) => {
    setFormatting(f => f.filter(x => x.id !== id));
  };
          ...
          <Paper sx={{ p: 3, borderRadius: 2 }}>
            ...
            <Box sx={{ mb: 2 }}>
              ...
              <Button
                ...
                onClick={() => {/* aplicar formatação condicional */}}
              >
